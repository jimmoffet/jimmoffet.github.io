...
class GameController {
  constructor() {
    ...
    this.startOverEarlySection = document.getElementById(
      "startOverEarlySection"
    );
    this.startOverEarlyButton = document.getElementById("startOverEarlyButton");
  ...
  restoreGameFromStorage(gameStartTime) {
    ...
    console.log(
      `Game restored: ${this.gameState.attemptClips.length} attempts, phase: ${this.gameState.phase}`
    );
  }
  ...
  createTestGame() {
    ...
    // Create test clips
    const targetClip = ClipMetadata.createTarget(
      gameStartTime,
      gameStartTime + 1000,
      "data:audio/webm;base64,test"
    );
    const attempt1 = ClipMetadata.createAttempt(
      gameStartTime,
      gameStartTime + 5000,
      "data:audio/webm;base64,test1",
      0
    );
    const attempt2 = ClipMetadata.createAttempt(
      gameStartTime,
      gameStartTime + 8000,
      "data:audio/webm;base64,test2",
      1
    );
    ...
    console.log(
      "Test game created. Reload page to see modal:",
  ...
  showRestoredGameInProgress() {
    ...
    this.startOverSection.style.display = "none"; // Hide "Play Again" button for in-progress games
  ...
  showGameRestoreModal() {
    ...
    if (this.gameState.isFinished()) {
      ...
      if (modalTitle)
        modalTitle.textContent = "👋 Hi! Continue your game or start over?";
      if (modalText)
        modalText.innerHTML = `You have a game in progress with <span id="attemptCount">${this.gameState.attemptClips.length}</span> copycats recorded.`;
    }
    ...
    if (updatedAttemptSpan)
      updatedAttemptSpan.textContent = this.gameState.attemptClips.length;
    ...
    console.log(
      `Showing restore modal for ${gameType} game with ${this.gameState.attemptClips.length} attempts`
    );
  }
  ...
  setupEventListeners() {
    ...
    // Early start over button - resets the game after target recording
    this.startOverEarlyButton.addEventListener("click", () =>
      this.startNewGame()
    );
    ...
    // Modal buttons
    this.continueGameButton.addEventListener("click", () =>
      this.continueExistingGame()
    );
    this.startOverModalButton.addEventListener("click", () =>
      this.startOverFromModal()
    );
  ...
  async startRecording() {
    try {
      ...
      this.mediaRecorder.ondataavailable = (event) => {
        console.log(
          "Game audio data chunk received:",
          event.data.size,
          "bytes"
        );
    ...
    try {
      ...
      reader.onloadend = () => {
        ...
        if (this.gameState.phase === GAME_CONFIG.PHASES.INITIAL) {
          ...
          this.gameState.phase === GAME_CONFIG.PHASES.TARGET_RECORDED
        ) {
  ...
  processTargetClip(base64Data, recordingStartTime) {
    ...
    // Show the early start over button
    this.startOverEarlySection.style.display = "block";

    // Start target playbook loop after 1 second delay
    setTimeout(() => {
      this.startTargetPlaybackLoop();
    }, 1000);
  }
  ...
  processAttemptClip(base64Data, recordingStartTime) {
    ...
    // Show finish button and hide early start over button after first attempt
    if (this.gameState.attemptClips.length === 1) {
      this.finishSection.style.display = "block";
      this.startOverEarlySection.style.display = "none";
    }
  ...
  startNewGame() {
    ...
    this.status.textContent =
      "Tap to record something for your friends to imitate!";
  ...
  showGameResults() {
    ...
    // Render clips list
    this.gameClipsList.innerHTML = allGameClips
      .map((clip, index) => {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (document.getElementById("gameClipsList")) {
    ...
    if (statusElement) {
      const permission =
        await MicrophonePermissions.ensurePermission(statusElement);
