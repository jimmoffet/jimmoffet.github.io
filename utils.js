...
class StorageService {
  ...
  static deleteClips(clipIds) {
    const existingClips = this.getAllClips();
    const filteredClips = existingClips.filter(
  ...
  /**
   * Delete all clips belonging to a specific game session
   */
  static deleteGameByStartTime(gameStartTime) {
    const allClips = this.getAllClips();
    const remainingClips = allClips.filter(
      (clip) => clip.gameStartTime !== gameStartTime
    );
    localStorage.setItem(this.STORAGE_KEY, JSON.stringify(remainingClips));

    const deletedCount = allClips.length - remainingClips.length;
    console.log(
      `Deleted game (${gameStartTime}): ${deletedCount} clips removed`
    );
    return deletedCount;
  }
...
class GameState {
  ...
  getAllClips() {
    return this.targetClip
      ? [this.targetClip, ...this.attemptClips]
      : [...this.attemptClips];
  }
  ...
  restoreFromStorage(gameStartTime) {
    ...
    const targetClip = gameClips.find(
      (clip) => clip.clipType === GAME_CONFIG.CLIP_TYPES.TARGET
    );
    ...
    if (currentStatus === "granted") {
      if (statusElement) {
        statusElement.textContent =
          statusElement.getAttribute("data-ready-text") || "Ready to record";
    ...
    if (result === "granted") {
      ...
      if (statusElement) {
        ...
        if (window.location.protocol === "file:") {
          statusElement.textContent =
            "🚫 Microphone requires HTTP/HTTPS. Try: http://localhost:3001";
        } else if (
