class AudioRecorder {
    constructor() {
        ...
        this.init();
        console.log('AudioRecorder initialized. Clips in storage:', this.clips.length);
    }
    ...
    async init() {
        try {
            ...
            if (this.status) this.status.textContent = 'Failed to initialize audio system';
    ...
    async loadClipDurations() {
        ...
        for (const clip of this.clips) {
            try {
                ...
                console.log(`Loaded duration for clip ${clip.id}: ${audioBuffer.duration.toFixed(2)}s`);
    ...
    async loadClipDuration(clipId) {
        ...
        try {
            ...
            console.log(`Loaded duration for new clip ${clipId}: ${audioBuffer.duration.toFixed(2)}s`);
    ...
    async startRecording() {
        try {
            ...
            this.mediaRecorder.onstart = () => {
                ...
                if (this.status) this.status.textContent = '🔴 Recording... Click to stop';
            };
    ...
    async processRecording() {
        try {
            ...
            reader.onloadend = () => {
                ...
                console.log('Clip saved to localStorage. Total clips:', this.clips.length);
                console.log('Clip details:', { id: clip.id, size: clip.size, timestamp: clip.timestamp });
                
                // Load the duration for the new clip
                this.loadClipDuration(clip.id);
                
                this.renderClips();
                if (this.status) this.status.textContent = 'Recording saved! Click to record again';
            };
    ...
    pausePlayback() {
        if (this.isPlaying && !this.isPaused && this.currentSource) {
            ...
            console.log('Calculated elapsed time:', this.audioContext.currentTime - this.startTime);
            ...
            console.log('State after pause - isPlaying:', this.isPlaying, 'isPaused:', this.isPaused);
    ...
    async resumePlayback() {
        if (this.isPaused && this.currentClipId && this.currentAudioBuffer) {
            ...
            console.log('Attempting to resume from pausedAt:', this.pausedAt, 'seconds');
            ...
            console.log('Current state - isPaused:', this.isPaused, 'isPlaying:', this.isPlaying);
            ...
            console.log('Calling source.start(0,', this.pausedAt, ',', remainingDuration, ')');
            ...
            console.log('This should make elapsed time calculation start at:', this.pausedAt);
            
            this.startTime = newStartTime;
            this.isPlaying = true;
            this.isPaused = false;
            
            console.log('State after resume - isPlaying:', this.isPlaying, 'isPaused:', this.isPaused);
            console.log('==================');
    ...
    updateTimeDisplay() {
        ...
        if (Math.floor(Date.now() / 1000) !== this.lastLogSecond) {
            ...
            console.log('Time update - AudioContext time:', this.audioContext.currentTime, 'Start time:', this.startTime, 'Elapsed:', elapsed);
        }
    ...
    renderClips() {
        ...
        this.clipsList.innerHTML = gameGroups.map((gameGroup, gameIndex) => {
            ...
            const targetClip = gameGroup.clips.find(clip => clip.clipType === 'target') || gameGroup.clips[0];
            const attemptClips = gameGroup.clips.filter(clip => clip.clipType === 'attempt') || gameGroup.clips.slice(1);
            ...
            <div class="game-group" data-game-id="${gameId}">
                ...
                <div class="game-summary">
                    <span class="clip-count">${attemptClips.length > 0 ? `${attemptClips.length} copycat${attemptClips.length !== 1 ? 's' : ''} 😻` : ''}</span>
    ...
    async startGamePlayAll(gameId) {
        ...
        // Find the game clips
        const gameGroups = this.groupClipsByGame();
        const gameGroup = gameGroups.find(group => `game-${group.gameStartTime}` === gameId);
        ...
        console.log('Starting game play all for:', gameId, 'clips:', this.gameClips.length);
    ...
    async playNextGameClip() {
        ...
        const logMessage = clip.clipType === 'target' ? 
    ...
    updatePlayAllButton(gameId, isPlaying) {
        const button = document.querySelector(`.play-all-button[data-game-id="${gameId}"]`);
        if (button) {
    ...
    deleteGame(gameId) {
        ...
        if (gameGroup) {
            ...
            console.log('Deleted game:', gameId, 'clips removed:', gameGroup.clips.length);
        }
...
function debugAudio() {
    ...
    if (recorder) {
        ...
        if (recorder.audioContext && recorder.audioContext.state === 'running') {
            ...
            gainNode.gain.exponentialRampToValueAtTime(0.01, recorder.audioContext.currentTime + 0.5);
...
document.addEventListener('click', () => {
    if (recorder && recorder.audioContext) {
        console.log('AudioContext state after user interaction:', recorder.audioContext.state);
    }
});
