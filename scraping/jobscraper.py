...
#parse("user experience", "boston")

if __name__ == "__main__":

	# eg-:python 1934_glassdoor.py "Android developer" "new york" "3"
	argparser = argparse.ArgumentParser()
	argparser.add_argument('keyword', help='job name', type=str)
	argparser.add_argument('place', help='job location', type=str)
	argparser.add_argument('days', help='max age of listing in days', type=str)
	...
	scraped_data = parse(keyword, place, days)
	print "Writing data to output file"

	with open('%s-%s-%s-days-old-job-results.csv' % (keyword, place, days), 'w')as csvfile:
		fieldnames = ['ID', 'Name', 'Company', 'State',
					  'City', 'Age', 'Salary', 'Location', 'Url']
